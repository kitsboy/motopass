    ...
    <span className="inline-flex flex-wrap items-center gap-1.5">
      {confirmed && proof.proof_url ? (
        <a
          ...
          title={formatT(t, 'trust.verifySatohash', { hash: proof.hash ?? '' })}
        >
          <BadgeCheck className="h-3 w-3 shrink-0" aria-hidden="true" />
          <span>{t('trust.bitcoinAnchored')}</span>
          <ExternalLink
        ...
        <span
          ...
          title={proof.ots_path ?? t('trust.proofOnFile')}
        >
          <BadgeCheck className="h-3 w-3 shrink-0" aria-hidden="true" />
          <span>{t('trust.bitcoinAnchored')}</span>
        </span>
        ...
        <span
          ...
          title={t('trust.proofPendingTitle')}
        >
          <Clock className="h-3 w-3 shrink-0" aria-hidden="true" />
          <span>{t('trust.proofPending')}</span>
        </span>
      )}

      {confirmed && (
        <span className="inline-flex items-center gap-1 font-mono text-[10px] text-mp-ink-tertiary">
          <span>{formatT(t, 'trust.block', { block: proof.block ?? '—' })}</span>
          {hashShort && (
            <span className="hidden sm:inline" title={formatT(t, 'trust.sha256', { hash: proof.hash ?? '' })}>
              · {hashShort}
            </span>
          )}
        </span>
