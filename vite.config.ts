...
(function(){var shown=false,retrying=false,KEY="motopass-cache-retry",SCROLL_KEY="motopass-cb-scroll",COUNT=3;
function bust(){try{sessionStorage.setItem(SCROLL_KEY,String(window.scrollY||0));}catch(e){}var u=new URL(location.href);u.searchParams.set("cb",String(Date.now()));location.replace(u.toString());}
try{var saved=sessionStorage.getItem(SCROLL_KEY);if(saved!=null){sessionStorage.removeItem(SCROLL_KEY);var y=parseInt(saved,10);if(!isNaN(y))requestAnimationFrame(function(){window.scrollTo(0,y);});}}catch(e){}
...
function recover(msg){if(shown||retrying)return;if(msg&&!poisoned(String(msg||""))){shown=true;showUI();return;}if(!sessionStorage.getItem(KEY)){retrying=true;showCountdown(COUNT,function(){sessionStorage.setItem(KEY,"1");retrying=false;bust();});return;}sessionStorage.removeItem(KEY);shown=true;showUI();}
