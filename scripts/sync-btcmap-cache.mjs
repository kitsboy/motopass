#!/usr/bin/env node
/** Per-jurisdiction BTC Map snapshots → public/data/btcmap/{slug}.json for offline use */
...
const OUT_DIR = resolve(__dirname, '../public/data/btcmap')
...
async function syncProgram(name, coord) {
  const q = new URLSearchParams({
    lat: String(coord.lat),
    lon: String(coord.lon),
    radius_km: String(coord.radiusKm),
  })
  const [places, areas] = await Promise.all([
    fetchJson(`/v4/places/search/?${q}`),
    fetchJson(`/v4/areas?lat=${coord.lat}&lon=${coord.lon}`),
  ])
  return {
    program: name,
    fetchedAt: new Date().toISOString(),
    coord,
    places: Array.isArray(places) ? places.slice(0, 48) : [],
...
async function main() {
  ...
  const manifest = { generatedAt: new Date().toISOString(), programs: [] }

  for (const [name, coord] of Object.entries(PROGRAM_COORDS)) {
    const file = `${slug(name)}.json`
    try {
      const snapshot = await syncProgram(name, coord)
      writeFileSync(resolve(OUT_DIR, file), JSON.stringify(snapshot))
      manifest.programs.push({ name, file, count: snapshot.places.length })
      process.stdout.write(`✓ ${file}: ${snapshot.places.length} places\n`)
    } catch (err) {
  ...
  console.log(`Wrote ${manifest.programs.length} snapshots to ${OUT_DIR}`)
}
