...
## 8. BTC Map Data (External + Cached)

MotoPass cross-references each jurisdiction with [BTC Map](https://btcmap.org) merchant data. Coordinates live in `src/data/programCoords.ts` (keyed by exact program `name` from `countries.json`).

### Density snapshot (`public/data/btcmap-density.json`)

```json
{
  "generatedAt": "2026-07-07T16:00:56.000Z",
  "programs": {
    "El Salvador": { "count": 1326, "tier": "dense", "score": 66 },
    "Uruguay": { "count": 8, "tier": "moderate", "score": 6 }
  }
}
```
...
|-------|------|-------------|
| `count` | number | Merchants within search radius |
| `tier` | `sparse` \| `moderate` \| `dense` | UI badge tier (&lt;5 / 5–19 / 20+) |
| `score` | number | Normalized density score 0–100 |

Generated by `npm run btcmap:density` (calls `GET /v4/places/search/` per jurisdiction).

### Offline cache (`public/data/btcmap/{slug}.json`)

Slug = lowercase program name with non-alphanumerics → hyphens (e.g. `el-salvador`, `uae-dubai-abu-dhabi`).

```json
{
  "program": "El Salvador",
  "fetchedAt": "2026-07-07T16:02:00.000Z",
  "coord": { "lat": 13.6929, "lon": -89.2182, "radiusKm": 80 },
  "places": [{ "id": 1, "lat": 13.69, "lon": -89.22, "name": "..." }],
  "areas": [{ "id": 600, "name": "El Salvador", "type": "country", "url_alias": "sv" }]
}
```

Generated by `npm run btcmap:sync`. Places capped at 48 per file. `manifest.json` indexes all snapshots.

### Live API types (`src/lib/btcmap.ts`)

- `BtcMapPlace` — id, lat, lon, name, address, verified_at, etc.
- `BtcMapArea` — country/community areas at a coordinate.
- Saved places require Nostr NIP-98 Bearer token (`/v4/places/saved`).

## 9. Implementation Checklist for New Fields
