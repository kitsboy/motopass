> **Note (BUILD-33):** Shipped app uses **Sovereign Night default** — glass cards (`GlassCard`), BTC grid/hash textures, canonical nav via `navRoutes.ts`. 18 routes including `/vault`, `/distressed`, `/apply`. Apply `src/styles/tokens.css` + `docs/DESIGN-CONTEXT.md`. Nav order: Programs · Vault · Distressed · BTC Map · Simulator · Compare · Agents · Apply. This prompt describes interaction structure; use current tokens when generating.
